...
describe("V2-SIM-006 analytical Roll and splitting", () => {
  ...
  it("AC-SIM-006-04 resolves pairwise and multi-event ties within 1e-9 seconds by precedence", () => {
...
describe("V2-SIM-007 Cup capture", () => {
  ...
  it("AC-SIM-007-02 requires exit and eligible re-entry after a fast traversal", () => {
    const result = resolveRoll({ ...base, speed: 2, cup: { x: 1, y: 0 } });
    expect(result.terminal).toBe("rest"); // slows while inside but no second capture attempt
    const reentry = resolveRoll({ ...base, speed: 2, cup: { x: 1.8, y: 0 } });
...
describe("V2-SIM-008/009 outcomes and resolved contract", () => {
  ...
  it("AC-SIM-008-01 restores/scoring for Carry and Roll Water/OOB while preserving Club/direction", () => {
    const rollWater = resolveShot({ shotId: "r", round, power: 1, originalLieTerrain: "green", cup: { x: -2, y: 0 }, terrainAt: (p) => p.x > 1 ? "water" : "green" });
    const carryOob = resolveShot({ shotId: "c", round: { ...round, selectedClub: "driver" }, power: 1, originalLieTerrain: "fairway", cup: { x: -2, y: 0 }, terrainAt: () => OUT_OF_BOUNDS });
    for (const shot of [rollWater, carryOob]) { expect(shot.resultingRound).toMatchObject({ lie: round.lie, playedStrokes: 3, penaltyStrokes: 2, selectedClub: shot.inputs.club, directionIndex: 0 }); }
  });
  ...
  it("AC-SIM-008-03 normalizes then reclassifies edge positions; live/resumed state is equal", () => {
    const shot = resolveShot({ shotId: "edge", round, power: 0.1, originalLieTerrain: "green", cup: { x: -2, y: 0 }, terrainAt: (p) => p.x >= 0.13 ? "water" : "green" });
    expect(shot.finalPosition.x).toBe(0); // hazard restores normalized pre-shot Lie
