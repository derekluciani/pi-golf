...
> [!WARNING]
> **Deprecated historical plan.** [`PRD.md`](PRD.md) supersedes this document as the sole normative product and acceptance specification for the Version 2 reimplementation. Do not derive implementation tickets or resolve requirements from this file.
...
This document records the original Version 1 design and is retained only for provenance. Domain terminology is defined in [`CONTEXT.md`](CONTEXT.md), current requirements in [`PRD.md`](PRD.md), and architectural rationale in [`adr/`](adr/).
